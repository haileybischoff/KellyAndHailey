...
uint32_t previousAlienColor = WHITE;

uint32_t alienColor = WHITE;

void setAlienColor(){
	previousAlienColor = alienColor;
	alienColor = getTankBulletColor();
}
...
uint8_t calculateBottomAlienBasedOnXPosition(point_t some_position){
	...
	uint8_t alienNumber = WRONG_ALIEN;
	point_t blockPosition = getAlienBlockPosition(); // This is getting the AlienBlockPosition
	/*This was suposed to try to ensure that an alien is killed even if the bullet wasn't under an alien when fired.
	if(x_position < blockPosition.x){ //The bullet is on the left of the alien columns
		uint8_t n = 0;
		for(n = 0; n < 11; n++){
			if((my_alien_row_dead[n] >= 0) && (my_alien_row_dead[n] < 55)){
				return my_alien_row_dead[n];
			}
		}
	}
	else if(x_position > (blockPosition.x + (11 * 32))){ //The bullet is on the right of the alien columns
		...
		if(alien_block[number]){ //If alien is drawn
			...
			for(line = 0; line < ALIEN_HEIGHT; line++){ //Height
				for(pixel = 0; pixel < ALIEN_WORD_WIDTH; pixel++){ //Width
					if(frame_pointer[(line + position.y)*SCREEN_WIDTH + (pixel + position.x)] == alienColor) //If the pixel is white
...
void eraseAlien(uint16_t x_position, uint16_t y_position){ //, uint8_t alien_type){ //Erase the alien
	...
	for(line = 0; line < ALIEN_HEIGHT; line++){ //Height
		for(pixel = 0; pixel < ALIEN_WORD_WIDTH; pixel++){ //Width
			if((frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == alienColor) || (frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == previousAlienColor)) //If the pixel is white
		...
		for(pixel = 0; pixel < ALIEN_WORD_WIDTH; pixel++){ //Width
			if((alien_explosion[line] & (SHIFT<<(ALIEN_WORD_WIDTH-SHIFT-pixel)))){ //If pixel is a 1
				if(frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == BLACK){ //If it is black
					frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] = alienColor; //Set to white
				}
			}
			else{
				if((frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == alienColor) || (frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == previousAlienColor)){ //If it is white
					frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] = BLACK; //Set to black
				}
			}
		}
...
void explosionKill(uint8_t deadAlien, bool add, uint8_t value){
	uint8_t alien_in_question;
	if(add){
		alien_in_question = deadAlien + value;
		if(isAlienAlive(alien_in_question)){
			killAlien(alien_in_question); //Kill Alien to the right
			computeScore(alien_in_question);
		}
	}
	else{
		alien_in_question = deadAlien - value;
		...
		for(pixel = 0; pixel < ALIEN_WORD_WIDTH; pixel++){ //Width
			if(alien_type == SQUID_IN){ //Squid in
				if((squid_step_in_symbol[line] & (SHIFT<<(ALIEN_WORD_WIDTH-SHIFT-pixel)))){
					if(frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] != alienColor){//Said not white
						frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] = alienColor; //Set to white
					}
				}
				else{
					if((frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == previousAlienColor) || (frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] == alienColor)){
						frame_pointer[(line + y_position)*SCREEN_WIDTH + (pixel + x_position)] = BLACK; //Set to black
					}
				}
			}
...
void eraseTopRow(){ //Erase the tops rows
	...
	for(line = 0; line < ALIEN_HEIGHT; line++){ //Height increased by the number of empty rows
		for(pixel = 0; pixel < ALIEN_BLOCK_WIDTH; pixel++){ //Width
			if((frame_pointer[(line + last_alien_position.y)*SCREEN_WIDTH + (pixel + last_alien_position.x)] == alienColor) || (frame_pointer[(line + last_alien_position.y)*SCREEN_WIDTH + (pixel + last_alien_position.x)] == previousAlienColor)){ //if it is white
