...
//This function is to load the FIFO
void alien_sound_method(int32_t SoundRate, int32_t Sound_length, int32_t Sound[], int32_t * count){
	uint32_t i;
	uint32_t sample = ZERO;
	XAC97_WriteReg(XPAR_AXI_AC97_0_BASEADDR, AC97_PCM_DAC_Rate, SoundRate); //Write the sound frequency
	for(i = 0; i < INCREMENT_SOUND; i++){ //We will be taking 100 samples
		(*count)++;
		sample = (Sound[*count] << SHIFT) | Sound[*count];
		XAC97_mSetInFifoData(XPAR_AXI_AC97_0_BASEADDR, sample); //Set the FIFO
		if(*count >= Sound_length){ //If we are at the end of the data
